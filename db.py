...
def _filters(params):
    clauses, values = [], []
    mapping = {"desde":("v.fecha_venta::date",">="), "hasta":("v.fecha_venta::date","<="), "region":("COALESCE(pr.nombre,'Sin provincia')","="), "canal":("cv.nombre_canal","=")}
    for key,(column,operator) in mapping.items():
        value=params.get(key)
        if value and value!="Todos": clauses.append(f"{column} {operator} %s"); values.append(value)
...
def dashboard(params=None):
    params=params or {}; extra,values=_filters(params); sale_where="WHERE v.estado_venta <> 'Cancelada'"+extra
    with connect() as conn:
        ...
        def sales_group(label,joins="",limit=12):
            ...
            {joins} {sale_where} GROUP BY 1 ORDER BY ingresos DESC LIMIT {limit}""",values)
        ...
        region=sales_group("COALESCE(pr.nombre,'Sin provincia')")
        ...
        COALESCE(pr.nombre,'Sin provincia') region,cv.nombre_canal canal,cl.nombre vendedor,SUM(d.cantidad)::int cantidad,v.total_venta::float total,v.estado_venta::text estado
        ...
        JOIN public.productos p ON p.id_producto=vp.id_producto {sale_where} GROUP BY v.id_venta,v.fecha_venta,pr.nombre,cv.nombre_canal,cl.nombre,v.total_venta,v.estado_venta
        ORDER BY v.id_venta DESC LIMIT 12""",values)
    return {"kpi":kpi,"tendencia":tendencia,"producto":producto,"categoria":categoria,"region":region,"canal":canal,"cliente":cliente,"inventario":inventario,"entrega":entrega,"recientes":recientes}
...
def catalog():
    with connect() as conn:
        ...
        "regiones":[r["nombre"] for r in _rows(conn,"SELECT nombre FROM public.provincia ORDER BY nombre")]}
